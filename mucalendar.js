...
function Calendar(element, options, eventSources) {
	...
	t.trigger = trigger;
    t.currentInterval = [];
    t.setCurrentInterval = setCurrentInterval;
    ...
    function setCurrentInterval(start, end) {
        ...
        if (typeof(start)=='object') {
            start = start.getTime();
        } else if (start < Math.pow(10, 11)) {
            start = start * 1000;
        }
        if (typeof(end) == 'object') {
            end = end.getTime();
        } else if (end < Math.pow(10, 11)) {
            end = end * 1000;
        }

        // Set interval:
        t.currentInterval = (start < end) ? [start, end] : [end, start];
    }
	...
	function reportEvents(_events) {
        var view = t.getView();
        if (t.currentInterval.length != 2 || typeof(view.getSlotSegmentContainer)!='function') {
            events = _events;
            clearSegments();
            renderEvents();
        } else {
            var container = view.getSlotSegmentContainer(),
                saved = [],
                start = t.currentInterval[0],
                stop  = t.currentInterval[1];
            events = [];
            $(_events).each(function(){
                var time = this._start.getTime();
                if (start <= time && time <= stop) {
                    events.push(this);
                } else {
                    var tmp = container.find('.fc-event[data-id="'+this._id+'"]');
            ...
            $(saved).each(function() {
                ...
                // Add events and elements:
                events.push(event);
                container.append(element);
            });

            // Bind data to the view:
            view.setEventData(_events);
        }
	}
...
function bindHandlersToSegs(segs, bindHandlers) {
    if (segs.length >= segments.length) {
        segments = segs;
    }
    else {
        segments = segs.concat(segments);
        segs.sort(compareSlotSegs);
    }

    $(segments).each(function(){
        // Unbind click event before adding new ones:
        this.element.unbind('click');
        // Bind events:
        bindHandlers(this.event, this.element, this);
    });
}
...
function AgendaEventRenderer() {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		bindHandlersToSegs(segs, bindSlotSeg);
...
function DayEventRenderer() {
	...
	function attachHandlers(segments, modifiedEventId) {
		...
		bindHandlersToSegs(segments, bindDaySeg);
	}
...
function ListView(element, calendar) {
	...
	function renderEvents(events, modifiedEventId) {
		...
		if (emptyMonth) {
			$("<li class='fc-agendaList-item'><span class='fc-agendaList-day'>Aucun evenement</span></li>").appendTo(html);
		}
		$(element).html(html);
